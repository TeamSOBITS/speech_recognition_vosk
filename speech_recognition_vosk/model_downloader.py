...
class ModelDownloader(Node):
    def __init__(self):
        ...
        self.lbl_welcome = tk.Label(self.model_frame, text='The VOSK model downloader for ROS!', font=self.gui_font)
    ...
    def download(self, model_to_download):
        ...
        if model_url is not None:
            ...
            if "Content-Length" in req.headers:
                ...
                total_size = None
            with open(os.path.join(self.model_dir, filename), "wb") as fileobj:
        ...
        for link in soup.findAll('a'):
            ...
            if model_link.startswith(self.url):
                if model_to_download + ".zip" in model_link:
                    file_name = model_link.split(self.url + '/', 1)[1]
